...
def playerStandings():
    ...
    """
    base_query_wins = """ CREATE VIEW v_wins AS
    ...
    base_query_losses = """ CREATE VIEW v_loss AS
    ...
    e = db.cursor()
    f = db.cursor()
    e.execute(base_query_wins + ';')
    f.execute(base_query_losses + ';')

    query_join = """
    select winners.id, winners.name, wins, wins+losses as matches
        from v_wins as winners left join v_loss as losers
...
def reportMatch(winner, loser):
    ...
    """    
    add_query = "INSERT INTO matches (winner_id, loser_id) VALUES(%s, %s);"
    db = connect()
    c = db.cursor()
    c.execute(add_query, (winner, loser,))
    db.commit()
    db.close()
